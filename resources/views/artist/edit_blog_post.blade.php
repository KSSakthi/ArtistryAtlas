...
<body>
    ...
    <h2 class="heading">Edit Blog Post</h2>
    <form action="{{ route('update.blog_post', $blog) }}" method="POST" enctype="multipart/form-data" id="blogForm">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <textarea id="blogContent" name="blogContent" rows="17" required>{{ $blog->content }}</textarea>
        </div>

        <div class="form-group">
            <label for="artworkImages">Blog Image:</label>
            <input type="file" id="artworkImages" name="artworkImages" accept="image/*" multiple>
            @if($blog->image_path)
                <img src="{{ asset('storage/' . $blog->image_path) }}" alt="Current Blog Image">
            @endif
        </div>
